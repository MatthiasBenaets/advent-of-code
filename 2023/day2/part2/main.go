...
const green = 13
const blue = 14;
var result, power int

func main() {
  for i := range data {
    result = result + calc(data[i])
  }
  fmt.Println(result)
}
  ...
  for _, minigames := range game[1:] {
    ...
    role := strings.Split(roles, ", ")
      for _, value := range role {
        ...
        // If number is > existing number in map, replace it
        if rolls[split[1]] < number {
          rolls[split[1]] = number
        }

        // Calculate the power of all numbers
        power = rolls["red"] * rolls["green"] * rolls["blue"]
      }
    }
  }
  return power
